...
public class intStack {
	...
	private int ptr;	// 스택 포언터 (현재까지 쌓여 있는 데이터의 수 나타냄)
	...
	public int push(int x) {	// 원하는 값(x)를 스택에 삽입, 0번 인덱스부터 차근차근 채우기
		stk[ptr++]=x;	// 포인터가 가리키는 위치에 값을 삽입한 후 포인터를 1 증가.
						// ptr++ : 데이터 개수 카운팅, 그 다음 데이터는 그다음칸에 들어갈 수 있도록
		return x; 	// 삽입된 데이터 반환
	}
	...
	public void clear() {	// 모든 데이터 전체 삭제
		ptr =  0;	// 포인터 : 현재 데이터 개수 -> 0 : 나는 아무것도 없다!
	}
	
	// 특정한 값 몇 번 인덱스에 있는지 찾기 : TOP(위에서부터)!!!
	// 스텍이 데이터를 찾을 때 위에서부터 
	public int indexOf(int x) {	// x : 찾고 싶으 값
		for(int i=ptr-1;i>=0;i--) {	// 제일 위에 있는 데이터 ~ 0번데이터까지 1한씩 내려오면서 탐색
			if(stk[i]==x) {		// 찾고자 하는 데이터와 같은 값을 발견했을 때
				return i;	// 그 데이터가 있는 인덱스 번호 반환
			}
		}
		return -1; // 찾고자 하는 값이 없을경우에는 -1반환
	...
	public boolean isFull() {
		return ptr==capacity;	// ptr 이 capacity같으면(가득차있으면) -> true 반환
								// capacity보다 적은 경우 -> false 반환
	}
