...
<html>
	...
	<body>
		...
		<header id="header" class="alt">
			...
			<nav>
				<c:choose>
					<c:when test="${empty loginMember}"> <!-- loginMember가 비어있는지 -->
						<a href="#menu">로그인</a>					
					</c:when>
					<c:otherwise>
						<c:if test="${loginMember.email eq 'admin'}">	<!-- equals->eq -->
		...
		<!-- Scripts -->
			...
			<script>
				function checkEmail(){
					...
					// innerHtml : 태그 사이의 html 요소 포함, html()
					// html() -> 값 가지고 오기, html("<h1>s</h1>") -> 값 추가하기
					// <span>텍스트</span>
					// innerText : 태그 사이의 텍스트, text()
					// 텍스트
					// <input>
					// value : input 태그에 작성된 값(사용자가입력한(선택한)값), val()
					let input = $("#input").val()
					console.log(input)
					// 비동기 통신 : json {"key","value"}, xml
					$.ajax({
						url : "member/emailcheck",	// 요청 경로
						type : "get",		 // http ㅌ요청 메서드(get(기본)/post)
						data : {'input' : input},
